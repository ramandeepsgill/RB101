...
def computer_places_piece!(brd)
  WINNING_LINES.each do |line|
    if ((brd.values_at(line[0], line[1], line[2]).count(PLAYER_MARKER) == 2) && (brd.values_at(line[0], line[1], line[2]).index(" ")))
      index = brd.values_at(line[0], line[1], line[2]).index(" ")
      puts "index is #{index}"
      square = line[index]
      brd[square] = COMPUTER_MARKER
      return
